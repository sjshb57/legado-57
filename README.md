...
4. 特殊章节处理（『第零章』→『序章』）
5. 支持纯数字标题转换（『1 内容』→『第1章 内容』）
6. 无标识符标题兼容（『第一百标题』→『第100章 标题』）
7. 多章节标识符适配（章/节/回/集等）
8. 灵活的配置系统（可自定义删除的标点字符集、默认章节后缀等）
9. 标识符白名单保护（指定标识符不被 defaultSuffix 覆盖）
10. 括号内容清理（白名单单字符保留并转为半角格式，其余括号整体删除）
```
...
```
『第1章（上）内容』    → 『第1章 (上) 内容』（白名单单字符，转为半角格式）
『第1章（特别版）内容』→ 『第1章内容』（非白名单，整体删除）
『第2章（上』        → 『第2章 (上)』（未闭合但白名单，补右括号转半角）
```
...
0. 括号内容处理 → 白名单单字符保留并转为半角格式，其余括号整体删除（含未闭合括号）
...
function processTitle(title) {
  ...
  // 括号内容处理
  // 白名单：一至十（汉字数字）及上中下，均为单字符
  // 闭合/未闭合括号统一处理：白名单字符转为半角格式，其余整体删除
  const bracketWhitelist = new Set('一二三四五六七八九十上中下');
  title = title.replace(/[（(]([^）)]*)[）)]|[（(]([^）)]*)$/g, (_, closed, open) => {
    const inner = closed ?? open;
    return (inner.length === 1 && bracketWhitelist.has(inner)) ? ' (' + inner + ')' : '';
  });
